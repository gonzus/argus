...
static void* buffer_realloc(void* ptr, Size len) {
    // realloc seems to work differently between MacOs and Linux. Since the intention when passing len==0 is to
    // free the memory, lets do it and return NULL, which is the expected behaviour
    if (len == 0) {
        free(ptr);
        return NULL;
    } else {
