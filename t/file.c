...
static void test_files(const char *dir, int expected) {
  ...
  do {
    argus = argus_create();
    if (!argus) {
      LOG_WARN("Could not create JSON parser");
      break;
    }

    dirp = opendir(dir_name);
    if (!dirp) {
      LOG_WARN("Could not open directory [%s]", dir_name);
      break;
    }
    ...
    while (!done) {
      ...
      if (!entry) {
        if (errno) {
          LOG_WARN("Could not read directory [%s]", dir_name);
        }
        break;
      }
  ...
  if (dirp) {
    ...
    if (ret) {
      LOG_WARN("Could not close directory [%s]", dir_name);
    }
    dirp = 0;
  }
...
static void test_good(void) { test_files("good", 1); }

static void test_bad(void) { test_files("bad", 0); }
