...
#define SHOW_STATE(msg, s) LOG_DEBUG("%s: %s", msg, STATE_NAME(s))
...
static char *StateName[STATE_LAST] = {
    "START",      "ARRAY_ELEM", "ARRAY_COMMA", "HASH_KEY", "HASH_COLON",
    "HASH_VALUE", "HASH_COMMA", "END",         "INVALID",
};

static int parse_string(Argus *argus, char quote, const char *ptr, int pos,
                        int len);
static int parse_number(Argus *argus, char c, const char *ptr, int pos,
                        int len);
static int parse_fixed(const char *fixed, int state, const char *ptr, int pos,
                       int len);
...
void argus_clear(Argus *argus) { stack_clear(argus->stack); }
...
int argus_parse_buffer(Argus *argus, const char *ptr, int len) {
  ...
  while (pos < len) {
    ...
    switch (c) {
    case '[':
      LOG_DEBUG("BOA %d", stack_size(argus->stack) + 1);
      current = state;
      switch (state) {
      case STATE_START:
    ...
    case ']':
      LOG_DEBUG("EOA %d", stack_size(argus->stack));
    ...
    case '{':
      LOG_DEBUG("BOH %d", stack_size(argus->stack) + 1);
      current = state;
      switch (state) {
      case STATE_START:
      case STATE_ARRAY_ELEM:
    ...
    case '}':
      LOG_DEBUG("EOH %d", stack_size(argus->stack));
      if (stack_pop(argus->stack, &current)) {
        LOG_INFO("UNDERFLOW");
        valid = 0;
        break;
  ...
  return (valid && pos == len && (state == STATE_END || state == STATE_START) &&
...
static int parse_string(Argus *argus, char quote, const char *ptr, int pos,
  ...
  while (pos < len) {
    unsigned char c = ptr[pos++];
    if (c == quote) {
      LOG_DEBUG("EOS [%d:%.*s]", argus->string->len, argus->string->len,
                argus->string->ptr);
      ok = 1;
      break;
    }
...
static int parse_number(Argus *argus, char c, const char *ptr, int pos,
  ...
  LOG_DEBUG("EON [%d:%d:%d]", sign, num[0], num[1]);
  (void)sign;
  return pos;
}
